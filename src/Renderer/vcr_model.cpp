...
namespace std {
template <> struct hash<vcr::Model::Vertex> {
    size_t operator()(vcr::Model::Vertex const& vertex) const {
        size_t seed = 0;
        vcr::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
        return seed;
    }
};
} // namespace std
