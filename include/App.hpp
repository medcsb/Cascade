...
const std::vector<const char*> deviceExtensions = {
    ...
    #define VALIDATION_MSG ""
#else
    const bool enableValidationLayers = true;
    #define VALIDATION_MSG "------Validation Layers Enabled------\n"
#endif
...
private:
    void initWindow() {
        ...
        window = glfwCreateWindow(WIDTH, HEIGHT, "My app", nullptr, nullptr);
    }
    ...
    void populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
        ...
        // define all the message severities and types that we want to capture
        // define from most severe to least severe
    ...
    bool isDeviceSuitable(VkPhysicalDevice device) {
        ...
        // Check if the device has the required GPU support
        if (!(deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU && deviceFeatures.geometryShader)) return false;

        QueueFamilyIndices indices = findQueueFamilies(device);
        // Check if the device has the required extension support
        bool extensionsSupported = checkDeviceExtensionSupport(device);
        if (!extensionsSupported) return false;
        // Check if the device has the required swap chain support
        SwapChainSupportDetails swapChainSupport = querySwapChainSupport(device);
        if (!swapChainSupport.isAdequate()) return false;
