...
const int MAX_FRAMES_IN_FLIGHT = 2; // Number of frames in flight
...
private:
    ...
    void cleanup() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) vkDestroySemaphore(device, renderFinishedSemaphores[i], nullptr);
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) vkDestroySemaphore(device, imageAvailableSemaphores[i], nullptr);
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) vkDestroyFence(device, inFlightFences[i], nullptr);
    ...
    void createGraphicsPipeline() {
        // vector of bytes
        //TOCHANGE: set path of compiled shaders
    ...
    void createSyncObjects() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            if (imageAvailableSemaphoreResult != VK_SUCCESS) throw std::runtime_error("failed to create image available semaphore!");
            if (renderFinishedSemaphoreResult != VK_SUCCESS) throw std::runtime_error("failed to create render finished semaphore!");
            if (fenceResult != VK_SUCCESS) throw std::runtime_error("failed to create fence!");
        }
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        // TOCHANGE: Clear color value (might be cool to change for background color)
        ...
        vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            ...
            //TOCHANGE: Draw command
            vkCmdDraw(commandBuffer, 3, 1, 0, 0); // Draw a triangle (3 vertices)
